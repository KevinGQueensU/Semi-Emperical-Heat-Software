...
class Atom:
    ...
    def __init__(self,
                 ...
                 f: float,          # Atomic fraction of medium
                 sig_file = None,   # Filepath with cross-sections (p,x) (nuclear reactions)
                 E_conv = 1,        # Conversion factor for (p, x) energy values to get eV
                 sig_conv = 1e-28   # Conversion factor for (p, x) cross-section values to get m^2 (default is barns -> m^2)
                 ):
        # Setting parameters
        self.name = name
        self.Z = Z # > 0
...
class Medium:
    ...
    def __init__(self,
                 ...
                 atoms: list[Atom] | Atom,              # Constituent atoms that make up the material
                 dEdx_filename: str,                    # SRIM file for stopping powers
                 Lx: float = 0, Ly = 0, Lz = 0,         # Dimensions of medium (optional)
                 name: str = None,                      # Medium name
                 x0: float = 0                          # Medium's start x position
                 ) -> None:
    ...
    def _compute_number_densities(self):
        # f values are weight fractions
        sum_w_over_A = sum(a.f / a.A for a in self.atoms)
        self.N_tot = (self.rho * 1e-3) * scipy.constants.N_A * 1e6 * sum_w_over_A

        # Per element number density: n_i = rho * N_A * w_i / A_i
        self.Nd = {a: (self.rho * 1e-3) * scipy.constants.N_A * 1e6 * a.f / a.A
                   for a in self.atoms}
    ...
    def get_C(self, T):
        if callable(self.C):
            ...
            return np.full_like(T, self.C, dtype=float)
